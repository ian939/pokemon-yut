...
포켓몬이 **윷판 위의 말**이 되는 가족 윷놀이예요. 처음엔 스타팅 포켓몬으로 시작하고, 윷판의 ❓ 풀숲에서 잡을 때마다 고를 수 있는 포켓몬이 늘어나요. 패드 한 대를 돌려 가며 가족끼리 하거나, 혼자서 로켓단과 대결해요.

**▶ 플레이: https://ian939.github.io/pokemon-yut/**

## 게임 방법

1. **👨‍👩‍👧 가족 대결** 또는 **😈 로켓단 대결**을 골라요 (말 2·3·4개, 빽도 켜기/끄기, 배틀 장면 ⚔️ 보기 / ⏭ 건너뛰기)
...
- **✨ 진화** — 말은 진화 1단계 모습으로 출발해서, 판을 돌며 **마지막 모습까지** 진화해요 (파이리 → 리자드 → 리자몽). 갈래가 여럿인 포켓몬(이브이 등)은 판마다 어느 쪽으로 진화할지 몰라요. 잡히면 처음 모습으로 돌아가요. 한 진화 가족에서는 한 마리씩만 골라요
...
- **🌿 ❓ 풀숲** — 판마다 윷판에 풀숲 2칸! 딱 멈추면 야생 포켓몬이 튀어나와요 (일반 50% · 레어 30% · 유니크 10% · 전설 10%). 가방의 볼을 골라 던지고(세 번 흔들리면 딸깍), 잡으면 **다음 판부터 말로** 쓸 수 있어요
...
```bash
node tools/test-rules.js          # 규칙 테스트 33개
