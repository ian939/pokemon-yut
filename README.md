...
- **📖 기술** — 마지막 모습이 되면 그 포켓몬 타입의 기술을 하나 무작위로 배워요 (전설은 처음부터). 내 차례에 **✨ 기술** 버튼으로 써요 — 말마다 한 판에 한 번, 한 차례에 하나. 니트로차지(바로 2칸)·파도타기(상대 2칸 뒤로)·지진·스텔스록·미래예지·희망사항… 모두 27개, 그중 카운터·철벽·달빛·길동무는 때가 되면 저절로 나가요. 처음 화면의 **✨ 기술 도감**에서 다 볼 수 있어요
...
| `docs/` | [기획서](docs/기획서.md) · [기술 스펙](docs/기술스펙.md) · [v2 계획](docs/v2-보물상자·야생포켓몬.md) · [v3 계획 — 기술·말 바꾸기](docs/v3-기술·말바꾸기.md) |

```bash
node tools/test-rules.js          # 규칙 테스트 56개 (기술 27개·말 바꾸기·무작위 3,000판)
node tools/sim-cpu.js             # 로켓단 난이도 시뮬레이션
python tools/test-ui.py           # 화면 검사 (Playwright)
```

`index.html`을 그냥 열어도(`file://`) 돌아가요. 개발용 주소 옵션: `?fast=1`(연출 빠르게) · `?seed=12` · `?force=5,4,3`(다음 윷 결과 정하기) · `?spots=3:58,12:133` / `?spots=none`(풀숲) · `?catch=1`/`0`(볼·로켓단 그물 결과) · `?box=master,poke,poke`(상자) · `?pools=nitro,surf|toxic,quake`(말마다 기술) · `?early=1`(모두 처음부터 기술) · `?swappool=rain`(바꿔 들어오는 포켓몬 기술).
